...
> AsanaNamedResourceArray get_favorites_for_user(user_gid, resource_type, workspace, opt_fields=opt_fields)
...
try:
    # Get a user's favorites
    api_response = api_instance.get_favorites_for_user(user_gid, resource_type, workspace, opt_fields=opt_fields)
    pprint(api_response)
